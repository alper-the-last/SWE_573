...
csv_file = open('C:\\Users\\HP\\Desktop\\BoğaziçiSWE\\SWE_573_Software Development Practice\\PyCharm Projects\\medsearch_django\\csv_file.csv','w',encoding='utf-8')
...
def save_to_db(x):
  for i in range (1, x, 1):
    ...
    for i in range(len(articles_data.get("PubmedArticle"))):
      if "Abstract" in articles_data["PubmedArticle"][i]["MedlineCitation"]["Article"]:
        csv_line = [getPMID(articles_data,i), getArticleTitle(articles_data,i), getArticleDate(articles_data,i), getAuthorList(articles_data,i),
                    getAbstractText(articles_data,i), getArticleKeywords(articles_data,i), getArticleLang(articles_data,i)]

        writer.writerow(csv_line)

        # all_article = Article(
        # PMID=getPMID(record2),
        # ArticleTitle=getArticleTitle(record2),
        # ArticleDate=getArticleDate(record2),
        # AuthorList=getAuthorList(record2),
        # AbstractText=getAbstractText(record2),
        # ArticleKeywords=getArticleKeywords(record2),
        # )
        #
        # all_article.save()
